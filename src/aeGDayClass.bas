...
'-------------------------------------------------|
'
' Ref: http://dailydoseofexcel.com/archives/2010/07/08/class-property-fixed-values/

' Reference to library "OLE Automation" (stdole) needed!

Private Const GUID_IPicture = "{7BF80980-BF32-101A-8BBB-00AA00300CAB}"    ' IPicture

' User-defined types: ----------------------------------------------------------------------

Public Enum aePicFileType
    aePicTypeBMP = 1
    aePicTypeGIF = 2
    aePicTypePNG = 3
    aePicTypeJPG = 4
...
Private Declare Function GdipCreateHICONFromBitmap_O Lib "ogl" Alias "GdipCreateHICONFromBitmap" (ByVal BITMAP As Long, hbmReturn As Long) As Long
...
Private Sub AutoShutDown()
    Set lclsTimer0 = New aeTimerClass
    ' Set to 5 seconds for next shutdown
    ' That's IMO appropriate for looped routines  - but configure for your own purposes
    If lGDIP <> 0 Then
        ' CANNOT USE AddressOf in VBA CLASS
        '''lTimer = SetTimer(0&, 0&, 5000, AddressOf TimerProc)
        Debug.Print "Timer0 StartTimer = " & lclsTimer0.GetStartTime
        Debug.Print "Timer0 EndTimer = " & lclsTimer0.GetEndTime
    End If
End Sub
...
Private Function ResampleGDIP(ByVal Image As StdPicture, ByVal Width As Long, ByVal Height As Long) As StdPicture
    Dim lRes As Long
    Dim lBitmap As Long
...
Private Function PicFromField(ByVal picField As DAO.Field, Optional FlattenColor As Variant = &HFFFFFFFF) As StdPicture
    Dim arrBin() As Byte
    Dim LSize As Long
...
Private Function ArrayToPicture(ByRef PicBin() As Byte, Optional FlattenColor As Variant) As StdPicture
    Dim IStm As IUnknown
    Dim lBitmap As Long
    Dim hBmp As Long
    Dim ret As Long
...
Private Function MaskFromPicture(ByVal Image As StdPicture, Optional TransColor As Variant) As StdPicture
    Dim lBitmap As Long
    Dim hBitmap As Long
    Dim W As Long, H As Long
    Dim bytes() As Long
...
Private Function LoadPictureGDIP_O(sFileName As String) As StdPicture
    Dim hBmp As Long
    Dim hPic As Long
...
Private Function ResampleGDIP_O(ByVal Image As StdPicture, ByVal Width As Long, ByVal Height As Long) As StdPicture
    Dim lRes As Long
    Dim lBitmap As Long
...
Private Function CropImage_O(ByVal Image As StdPicture, _
                   X As Long, Y As Long, _
                   Width As Long, Height As Long) As StdPicture
    Dim ret As Long
    Dim lBitmap As Long
...
Private Function GetDimensionsGDIP_O(ByVal Image As StdPicture) As TSize
    Dim lRes As Long
    Dim lBitmap As Long
    Dim X As Long, Y As Long
...
Private Function ArrayToPicture_O(ByRef PicBin() As Byte, Optional FlattenColor As Variant) As StdPicture
    Dim IStm As IUnknown
    Dim lBitmap As Long
    Dim hBmp As Long
    Dim ret As Long
...
Private Function MaskFromPicture_O(ByVal Image As StdPicture, Optional TransColor As Variant) As StdPicture
    Dim lBitmap As Long
    Dim hBitmap As Long
    Dim W As Long, H As Long
    Dim bytes() As Long
